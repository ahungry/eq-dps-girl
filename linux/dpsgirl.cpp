...
void setClippingRegion(GtkWidget *window){
  ...
  // if (points.empty()) {
  //       return; // Nothing to clip if no points are provided
  //   }

  //   cairo_region_t *shape = cairo_region_create();
  //   cairo_t *cr = cairo_create(nullptr); // Create a temporary Cairo context

  //   // Begin defining the path for the polygon
  //   cairo_new_path(cr);
  //   cairo_move_to(cr, points[0].x, points[0].y); // Move to the first point

  //   // Add lines to connect the subsequent points
  //   for (size_t i = 1; i < points.size(); ++i) {
  //       cairo_line_to(cr, points[i].x, points[i].y);
  //   }

  //   cairo_close_path(cr); // Close the polygon by connecting the last point to the first

  //   // Create a region from the filled path
  //   cairo_region_t *polygon_region = cairo_region_create_from_polygon
  //     (points[0].x, points[0].y, points.size(), (cairo_line_join_t*)points.data());
  //   cairo_region_union(shape, polygon_region); // Combine with the existing (empty) region
  ...
  //   cairo_region_destroy(shape);
  //   cairo_region_destroy(polygon_region);
  //   cairo_destroy(cr);
}
